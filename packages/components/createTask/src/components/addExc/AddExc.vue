<template>
	<a-modal
		...
		<div class="exc-box">
			...
			<div
				class="exc-list"
				:class="{'dis-bgc': isDisable}"
				@click="!isDisable && handleAddExc()">
				<span v-if="executeUser?.length == 0" class="empty-text">请选择</span>
				<div
					class="exc-block"
					v-for="item in executeUser"
					:key="item.userId"
					@click.stop>
					...
					<iconpark-icon
						...
						class="close-icon"
						@click.stop="
							!isDisable && handleDeleteUser(item.userId)
						"></iconpark-icon>
				</div>
		...
		<div class="exc-abort-time">
			...
			<iconpark-icon
				...
				class="delete-icon"></iconpark-icon>
		</div>
	</a-modal>
...
const initData = () => {
	if (!checkNullObj(props.subTask)) {
		...
		dayFormat.value = props.subTask.abortTime
			? dayjs(props.subTask.abortTime)
			: undefined;
	} else {
		excFrom.content = '';
...
const handleCancelEvent = () => {
	...
	if (props.isDisable || cache.openData === cache.closeData) {
		...
		dialog.title = '关闭';
		dialog.content = `确定放弃${
			checkNullObj(props.subTask) ? '添加' : '修改'
		}参与人吗？`;
	}
};
