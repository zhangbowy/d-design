<template>
	<a-drawer
		...
		<template #title>
			<div class="task-detail-title">
				...
				<a-popover
					v-if="
						taskDetail?.status === 'NOT_BEGIN' ||
						(isShowDelete && (role === 'CREATE' || role === 'FOR_CREATE'))
					"
			...
			<a-spin v-if="taskDetail?.type === 'MAIN_TASK'" :spinning="spinning">
				<div class="upload-box">
					<div class="file-box">
						<div
							class="file-list"
							v-for="item in accessory.ossAccessory"
		...
		<div class="main-task-text" v-if="taskDetail?.mainTaskContent">
			<span class="main-task-content" @click="handleTaskTrace">{{
				taskDetail?.mainTaskContent
			}}</span>
			<div
				v-if="accessory.ossAccessory.length > 0"
				class="file-btn"
				@click="handleCheckFiles">
		...
		<div class="add-principal">
			...
			<div
				...
				<div v-if="checkNullObj(principalUser)" class="empty-text">
					{{ role == 'EXECUTE' || status != 'NOT_BEGIN' ? '无' : '选择负责人' }}
				</div>
				<div v-else class="user-box">
		...
		<div class="add-deadline">
			...
			<div
				v-if="dayFormat || status == 'NOT_BEGIN'"
				class="deadline-box"
				:class="{
					'dis-bgc': role == 'EXECUTE' || status != 'NOT_BEGIN',
					'dis-text': role == 'EXECUTE' && !checkCanUpdateTime(role),
				}">
				<template
					v-if="role == 'EXECUTE' && dayFormat && !checkCanUpdateTime(role)">
			...
			<template v-else="">
				<iconpark-icon
					name="delete"
					class="delete-icon"
					v-if="status == 'NOT_BEGIN' && dayFormat"
		...
		<div class="add-remind" v-if="dayFormat != null && status == 'NOT_BEGIN'">
					...
					<div
						...
						<a-select
							...
							<a-select-option
								...
								:key="item.type"
								:value="item.type"
								>{{ item.customTime }}</a-select-option
							>
							<a-select-option class="custom-option">
								<div class="custom-time">
									...
									<a-date-picker
										...
										:show-time="{
											defaultValue: dayjs('00:00:00', 'HH:mm:00'),
										}"
										:allowClear="false"
										:autofocus="false"
										:showNow="false"
			...
			<!-- add cycle -->
			<div
				v-if="cycleValue && role != 'EXECUTE' && trait === 'default'"
				class="cycle-box">
		...
		<div class="add-exc" v-if="role != 'EXECUTE'">
			...
			<span
				class="empty-exc"
				v-if="status != 'NOT_BEGIN' && subTasks.length === 0"
				>无</span
			>
			<div class="exc-list">
					...
					<div class="exc-main" @click="handleCheckExc(item)">
						...
						<div class="main-right">
							<div class="user-name">
								...
								<span v-if="item.executeUser.length > 2"
							...
							<div class="task-data">
								<span v-if="item.leaderAbortTime" class="deadline-time"
									>截止{{ returnTIme(item.leaderAbortTime) }}</span
								>
			...
			<Transition>
				<div class="add-footer" v-if="!isEdit">
					<a-button
						v-if="showDetail"
						class="cancel-btn"
						@click="handleTaskTrace"
						>任务追踪</a-button
					>
					<a-button
						v-if="taskDetail.role != 'INDEPENDENT'"
						type="primary"
						class="sure-btn"
						:class="{'restart-btn': status != 'NOT_BEGIN'}"
						@click="handleReloadTask(false)"
						>{{ status == 'NOT_BEGIN' ? '完成任务' : '重启任务' }}</a-button
					>
				</div>
				<div class="save-footer" v-else>
					<a-button class="cancel-btn" @click="handleDrawerClose"
						>取消</a-button
					>
					<a-button
						type="primary"
						class="sure-btn"
						:disabled="judgeStrNull(content)"
						:loading="loading"
	...
	<!-- upload file list -->
	<FileListVue
		:appendixShow="files.show"
		:accessory="files.accessory"
	...
	<LookRelation
		...
		:tabs="['OKR', 'PROJECT']"
		:info="lookRelation.info"
		:isSureRelation="trait !== 'PROJECT'"
		:isOperate="
			taskDetail?.role != 'INDEPENDENT' && taskDetail?.role != 'EXECUTE'
		"
		@lookDetailCallback="lookDetailCallback"
...
const renderCorText = () => {
	...
	if (projects + okrs > 0) {
		return `已关联${projects > 0 ? `${projects}个项目` : ''}${
			...
			(projects > 0 && tasks > 0) || (okrs > 0 && tasks > 0) ? '、' : ''
		}${tasks > 0 ? `${okrs}个任务` : ''}`;
	} else {
		return '';
	}
...
const handleCheckRelation = () => {
	...
	lookRelation.info.type =
		props.taskDetail.type === 'MAIN_TASK' ? 'TASK_MAIN' : 'TASK_SUB';
...
const judgeCorBtnShow = () => {
	...
	if (props.trait === 'OKR' || props.trait === 'INTE') {
		if (
			props.taskDetail?.role != 'EXECUTE' &&
			props.taskDetail?.role != 'INDEPENDENT'
		) {
			bol = true;
		}
