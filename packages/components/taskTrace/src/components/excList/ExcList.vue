      ...
      <a-tabs v-model:activeKey="activeKey" @change="handleChangeTab">
        ...
        >
          ...
          v-if="
            ...
            execList['NOT_DONE'].length > 0 && item.name == 'NOT_DONE' && taskData.role != 'EXECUTE'
          "
...
const handlePress = () => {
  ...
  handleRemindDD(users, content, props.taskData.id, porps.trait, true, keyEventUrgeCommand);
};
</script>
