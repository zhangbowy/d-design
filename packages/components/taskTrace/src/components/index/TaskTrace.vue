      ...
      >
        ...
        <div class="bd" ref="divRef">
          <div class="comment-item" v-for="item in commentList" :key="item">
            ...
            >
              ...
              <span class="file-list">附件{{ returnFilesCnt(item.accessory) }}个</span>
            </div>
          </div>
        </div>
        <div class="comments-operation">
          ...
          <div class="btns" v-if="commentValue">
            ...
            <a-button size="small" type="primary" :disabled="isShow" @click="submitComment"
              >提交</a-button
            >
          </div>
        </div>
...
import AvatarGroupVue from "../avatar/AvatarGroup.vue";
...
const handleDDRemind = (user, content, read = true) => {
  ...
  handleRemindDD(user, content, props.taskData.id, props.trait, true, keyEventUrgeCommand);
};
 ...
 */
 const updateContent = (val) => {
  content.value = val;
  if(content.value.replace(/(^\s*)|(\s*$)/g, "") == `回复${replyUserInfo.name}`) {
    isShow.value = true;
  } else {
    isShow.value = false;
  }
